...
local function isWithinTargetZone(position, size)
    ...
    -- Calculate if the mouse is within the target area for the current body part
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            if isMouseOnTarget(TargetPlayer) then
                ...
                for _, part in pairs(bodyParts) do
                    ...
                    -- Get the part's position and check if the mouse is within the target area
                    if isWithinTargetZone(part.Position, size) then
                        local screenPos, onScreen = Camera:WorldToViewportPoint(part.Position)
                        syncBoxWithTarget(screenPos)
                        
                        -- Now we need to click any pixel inside the part's bounding box
                        local mouseX, mouseY = getMousePosition()

                        -- Shoot if mouse is within the bounding area of the body part
                            ...
                            if tool and tool:IsA("Tool") then
                                if not isIgnoringKnife() then
                                    ...
                                    if shootFunction and shootFunction:IsA("RemoteEvent") then
                                        ...
                                        -- Fire at any pixel within the part's bounding box
